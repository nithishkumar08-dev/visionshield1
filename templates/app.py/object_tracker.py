...
#MP
from logic.analyze import analyze_file
#EMP
...
def main(_argv):
    ...
    while True:
        ...
        # if enable info flag then print details about each track
            if FLAGS.info:
                mp_frame_data += f'Tracker ID: {str(track.track_id)}, Class: {class_name},  BBox Coords (xmin, ymin, xmax, ymax): {(int(bbox[0]), int(bbox[1]), int(bbox[2]), int(bbox[3]))}\n'
                if print_allowed:
